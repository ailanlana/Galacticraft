...
public class GuiOxygenCompressor extends GuiContainerGC {
    private static final ResourceLocation compressorTexture =
            new ResourceLocation(GalacticraftCore.ASSET_PREFIX, "textures/gui/compressor.png");
    ...
    public GuiOxygenCompressor(
            InventoryPlayer par1InventoryPlayer, TileEntityOxygenCompressor par2TileEntityAirDistributor) {
    ...
    public void initGui() {
        ...
        oxygenDesc.add(EnumColor.YELLOW + GCCoreUtil.translate("gui.oxygenStorage.desc.1") + ": "
                + ((int) Math.floor(this.compressor.storedOxygen) + " / "
                        + (int) Math.floor(this.compressor.maxOxygen)));
        ...
        electricityDesc.add(EnumColor.YELLOW
                ...
                + ((int) Math.floor(this.compressor.getEnergyStoredGC()) + " / "
                        + (int) Math.floor(this.compressor.getMaxEnergyStoredGC())));
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        status = GCCoreUtil.translate("gui.oxygenUse.desc") + ": " + TileEntityOxygenCompressor.TANK_TRANSFER_SPEED * 20
                + GCCoreUtil.translate("gui.perSecond");
        this.fontRendererObj.drawString(
                status, this.xSize / 2 - this.fontRendererObj.getStringWidth(status) / 2, 60, 4210752);
        // status = ElectricityDisplay.getDisplay(this.compressor.ueWattsPerTick * 20,
        // ElectricUnit.WATT);
    ...
    private String getStatus() {
        if (this.compressor.getStackInSlot(0) == null
                || !(this.compressor.getStackInSlot(0).getItem() instanceof ItemOxygenTank)) {
            return EnumColor.DARK_RED + GCCoreUtil.translate("gui.status.missingtank.name");
        }

        if (this.compressor.getStackInSlot(0) != null
                && this.compressor.getStackInSlot(0).getItemDamage() == 0) {
    ...
    protected void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) {
        ...
        if (this.compressor != null) {
            ...
            EnergyDisplayHelper.getEnergyDisplayTooltip(
                    this.compressor.getEnergyStoredGC(), this.compressor.getMaxEnergyStoredGC(), electricityDesc);
            // electricityDesc.add(EnumColor.YELLOW +
            // GCCoreUtil.translate("gui.energyStorage.desc.1") + ((int)
            // Math.floor(this.compressor.getEnergyStoredGC()) + " / " + (int)
            // Math.floor(this.compressor.getMaxEnergyStoredGC())));
            this.electricInfoRegion.tooltipStrings = electricityDesc;
        }
    }
}
